...
u32 C[17];//存放子秘钥前28位（从左往右），最后4位补0
u32 D[17];//存放子秘钥后28位（从左往右），最后4位补0
u32 L[17];//存放IP变换前32位（从左往右）
u32 R[17];//存放IP变换后32位（从左往右）
...
*output:1字节数据，低4位有效
**************************************************/
static u8 S_Inverse(u8 data,u8 *s_table)
{
	u8 row;
	u8 column;
	row = ((data&0x20)>>4)|(data&0x01);//行
	column = (data&0x1E)>>1;			//列
	//return s_table[row][column];		//返回数据低4位有效
...
static void IPExchange(u8 *plaintext)
{
	...
	L[0] = ((u32)temp[0]<<24)|((u32)temp[1]<<16)|((u32)temp[2]<<8)|((u32)temp[3]);
	R[0] = ((u32)temp[4]<<24)|((u32)temp[5]<<16)|((u32)temp[6]<<8)|((u32)temp[7]);
	for(i=1;i<=DES_ENCRYPT_NUM;i++)
	{
		L[i]=R[i-1];
		R[i]=L[i-1]^f(R[i-1],&KEY[i][0]);
	}
}
...
void DES(u8 *plaintext,u8 *key, u8 *ciphertext)
{
	...
	for(i=0;i<64;i++)//进行最后的置换，生成密文
	{
		j = IP_1[i]/8;//对应字节
		if(j<4)
			ciphertext[i/8] |= (u8)((R[16]>>(31-IP_1[i]%32))&0x01);
		else
			ciphertext[i/8] |= (u8)((L[16]>>(31-IP_1[i]%32))&0x01);
		if((i%8) != 7)
